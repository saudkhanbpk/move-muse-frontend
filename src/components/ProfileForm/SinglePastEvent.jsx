...
import papershape from "../../img/icons/papershape.png";
...
const SinglePastEvent = () => {
  ...
  useEffect(() => {
    const fetchFutureEvents = async () => {
      try {
        ...
        setIsFavourite(response.data.event.favourite);
  ...
  const handleFavouriteClick = async () => {
    try {
      const response = await axios.put(
        `${BaseUrl}/api/v1/events/favourite-past/${id}`,
        { favourite: !isFavourite }
      );
      setIsFavourite(response.data.data.favourite);
      toast.success(
  ...
  const handleFlagSubmit = async () => {
    try {
      const response = await axios.put(
        `${BaseUrl}/api/v1/events/past-event-update-flage/${id}`,
        { flagged: true, message: flagMessage }
      );
      setIsFlagged(response.data.data.flagged);
            ...
            <div className="col-12 col-md-6 d-md-flex justify-content-md-end">
              ...
              >
                ...
                <div className="">
                  <img
                    src={
                      isFavourite ? blog_heart_selected : blog_heart_unselected
                    }
