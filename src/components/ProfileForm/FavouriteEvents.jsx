...
const FavouriteEvents = () => {
  ...
  // Fetch only favorite events
  const fetchFavoriteEvents = async () => {
    try {
      const response = await axios.get(`${BaseUrl}/api/v1/events/future-events`);
      // Filter events where favorite is true
      const favoriteEvents = response.data.filter(event => event.favourite === true);
  ...
  useEffect(() => {
    fetchFavoriteEvents(); // Fetch only favorite events
  }, []);
